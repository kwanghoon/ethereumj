...
public class Lexer {
	public static void LexicalAnalysis(CommonParserUtil<Token> pu) {
		...
		pu.lexKeyword("assert", text -> { return Token.ASSERT; });
		pu.lexKeyword("balance", text -> { return Token.BALANCE; });
		pu.lexKeyword("by", text -> { return Token.BY; });
		pu.lexKeyword("value", text -> { return Token.VALUE; });
